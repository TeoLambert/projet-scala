...
object Canvas {
  ...
  def createCanvas(arguments: Seq[String], canvas: Canvas): (Canvas, Status) = 
    if (arguments.size < 3) 
      (canvas, Status(error = true, message = s"La commande new_canvas attend 3 arguments. Or vous en avez donné ${arguments.size}. Voici un exemple de commande valide: new_canvas 10 10 ."))
    // check if the second and third arguments are integers and positive and the third is a char
    else if (!arguments(0).forall(_.isDigit) || !arguments(1).forall(_.isDigit) || arguments(2).size != 1)
      (canvas, Status(error = true, message = "Il y a une erreur de type dans les arguments. Voici un exemple de commande valide: new_canvas 10 10 ."))
    else  {
      val dummyCanvas = Canvas(
  ...
  def loadImage(arguments: Seq[String], canvas: Canvas): (Canvas, Status) =
    if (arguments.size < 1)
      (canvas, Status(error = true, message = "La commande load_image attend 1 argument. Or vous en avez donné 0"))
    else {
  ...
  def updatePixel(arguments: Seq[String], canvas: Canvas): (Canvas, Status) = {
    if (arguments.size < 3) {
      (canvas, Status(error = true, message = s"La commande update_pixel attend 3 arguments. Or vous en avez donné ${arguments.size}. Voici un exemple de commande valide: update_pixel 0 0 ."))
    }
  ...
  def draw(arguments: Seq[String], canvas: Canvas): (Canvas, Status) = 
    if (arguments.size < 2) 
      (canvas, Status(error = true, message = s"La commande draw attend au moins 2 arguments. Or vous en avez donné ${arguments.size}."))
    else {
      arguments(0) match {
  ...
  def drawRectangle(arguments: Seq[String], canvas: Canvas): (Canvas, Status) = {
    if (arguments.size < 4) {
      return (canvas, Status(error = true, message = s"La commande rectangle attend 3 arguments. Or vous en avez donné ${arguments.size}."))
    }
  ...
  def drawTriangle(arguments: Seq[String], canvas: Canvas): (Canvas, Status) = {
    if (arguments.size < 4) {
      return (canvas, Status(error = true, message = s"La commande triangle attend 3 arguments. Or vous en avez donné ${arguments.size}."))
    }
  ...
  def fill(arguments: Seq[String], canvas: Canvas): (Canvas, Status) = {
    if (arguments.size < 3) {
      return (canvas, Status(error = true, message = s"La commande fill attend 2 arguments. Or vous en avez donné ${arguments.size}."))
    }
