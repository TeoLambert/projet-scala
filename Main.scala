...
case class Canvas(
  ...
  def update(pixel: Pixel): Canvas = {
    ...
    val newPixels =
      pixels.updated(pixel.y, pixels(pixel.y).updated(pixel.x, pixel))
...
object Canvas {
  ...
  def createCanvas(arguments: Seq[String], canvas: Canvas): (Canvas, Status) =
    if (arguments.size < 3)
      ...
      val dummyCanvas = Canvas(
        ...
        pixels = Vector.fill(arguments(1).toInt)(
          Vector.fill(arguments(0).toInt)(Pixel(0, 0, arguments(2).head))
        )
      )
      (dummyCanvas, Status())
    }
    ...
    if (arguments.size < 1)
      (
        ...
        Status(
          error = true,
          message =
            "La commande load_image attend 1 argument. Or vous en avez donné 0"
        )
      )
    else {
        ...
        case e: Exception =>
          (
            ...
            Status(
              error = true,
              message =
                s"Erreur de chargement de l'image: $e. Vérifiez que le fichier existe et que vous avez les droits d'accès."
            )
          )
      }
    }
    ...
    if (arguments.size < 3) {
      (
        ...
        Status(
          error = true,
          message =
            s"La commande update_pixel attend 3 arguments. Or vous en avez donné ${arguments.size}. Voici un exemple de commande valide: update_pixel 0 0 ."
        )
      )
    }

    val newCanvas = canvas.update(
      Pixel(arguments(0).toInt, arguments(1).toInt, arguments(2).head)
    )
    ...
    if (arguments.size < 2)
      (
        ...
        Status(
          error = true,
          message =
            s"La commande draw attend au moins 2 arguments. Or vous en avez donné ${arguments.size}."
        )
      )
    else {
      arguments(0) match {
        ...
        case _ =>
          (
            ...
            Status(
              error = true,
              message =
                s"La commande ${arguments(0)} n'existe pas. Essayez une de ces commandes line, rectangle, triangle, polygon ou fill."
            )
          )
      }
    }
  ...
  def drawLine(arguments: Seq[String], canvas: Canvas): (Canvas, Status) = {
    if (arguments.size < 4) {
      return (
        ...
        Status(
          error = true,
          message =
            s"La commande line attend 3 arguments. Or vous en avez donné ${arguments.size}."
        )
      )
    }
    ...
    if (
      x1 < 0 || x1 >= canvas.width || y1 < 0 || y1 >= canvas.height || x2 < 0 || x2 >= canvas.width || y2 < 0 || y2 >= canvas.height
    ) {
      return (
        canvas,
        Status(
          error = true,
          message =
            s"Vous avez essayé de dessiner une ligne en dehors du canvas."
        )
      )
    }
  ...
  def drawRectangle(
    ...
    if (arguments.size < 4) {
      return (
        ...
        Status(
          error = true,
          message =
            s"La commande rectangle attend 3 arguments. Or vous en avez donné ${arguments.size}."
        )
      )
    }
    ...
    val (x1, y1, x2, y2) = (
      math.min(pixel1.x, pixel2.x),
      math.min(pixel1.y, pixel2.y),
      math.max(pixel1.x, pixel2.x),
      math.max(pixel1.y, pixel2.y)
    )

    var newCanvas = canvas
    newCanvas =
      drawLine(Seq("line", s"$x1,$y1", s"$x2,$y1", s"$color"), newCanvas)._1
    newCanvas =
      drawLine(Seq("line", s"$x1,$y2", s"$x2,$y2", s"$color"), newCanvas)._1
    newCanvas =
      drawLine(Seq("line", s"$x1,$y1", s"$x1,$y2", s"$color"), newCanvas)._1
    newCanvas =
      drawLine(Seq("line", s"$x2,$y1", s"$x2,$y2", s"$color"), newCanvas)._1
    ...
    if (arguments.size < 4) {
      return (
        ...
        Status(
          error = true,
          message =
            s"La commande triangle attend 3 arguments. Or vous en avez donné ${arguments.size}."
        )
      )
    }
  ...
  def drawPolygon(arguments: Seq[String], canvas: Canvas): (Canvas, Status) = {
    if (arguments.size < 3) {
      return (
        ...
        Status(
          error = true,
          message = "La commande polygon attend au moins 2 arguments."
        )
      )
    }
    ...
    for (i <- 1 until arguments.size - 2) {
      val (newCanvas1, _) = drawLine(
        Seq("line", arguments(i), arguments(i + 1), color.toString),
        newCanvas
      )
      newCanvas = newCanvas1
    }
    val (newCanvas2, _) = drawLine(
      Seq("line", arguments(arguments.size - 2), arguments(1), color.toString),
      newCanvas
    )
    (newCanvas2, Status())
  }
  ...
  def fill(arguments: Seq[String], canvas: Canvas): (Canvas, Status) = {
    if (arguments.size < 3) {
      return (
        ...
        Status(
          error = true,
          message =
            s"La commande fill attend 2 arguments. Or vous en avez donné ${arguments.size}."
        )
      )
    }
    ...
    if (
      pixel.x < 0 || pixel.x >= canvas.width || pixel.y < 0 || pixel.y >= canvas.height
    ) {
      return (
        canvas,
        Status(
          error = true,
          message = s"Vous avez donné des coordonnées en dehors du canvas."
        )
      )
    }
    ...
    val updatedPixels =
      fillRecursive(pixel.x, pixel.y, targetColor, color, canvas)
  ...
  def fillRecursive(
    ...
    if (
      x < 0 || x >= canvas.width || y < 0 || y >= canvas.height || canvas
        .pixels(y)(x)
        .color != targetColor
    ) {
      canvas
      ...
      Seq((x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1))
        .foldLeft(canvas.update(Pixel(x, y, color))) { (canvas, coords) =>
          val (xx, yy) = coords
